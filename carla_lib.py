...
import pygame
import queue
#from PythonAPI.examples.client_bounding_boxes import VIEW_HEIGHT
...
class Sensors:
    class Camera(object):
        def __init__(self, world, vehicle , spawn_point, reder_pos = (0,0), render_dimensions = (640, 360), attach_to = None, type = 0):
            #params
            self.type = type
            ...
            #camera data
            self.surface_queue = queue.Queue()
            #self.image_queue = queue.Queue() #3d matrix of the rgb data of the camera 
        ...
        def set_camera_pos(self, location, rotation = carla.Rotation(0,0,0)):
            ...
            location = carla.Location(box.location) + location
        ...
        def parse_image(self, image):
            ...
            array = array[:, :, ::-1]
            self.surface_queue.put(pygame.surfarray.make_surface(array.swapaxes(0, 1)))
            
        #render current sensor data to display
        def render(self, display):
            display.blit(self.surface_queue.get(), self.render_pos)
...
class World:
    ...
    def spawn_camera(self, spawn_point, render_pos = (0,0), render_dimentions = (640, 360), attach_to = None, type = 0):
        #spawn camera, z up and down, y left and right, x forward and backward 
        self.sensor_list.append(Sensors.Camera(self.world, self.vehicle, spawn_point, render_pos, render_dimentions, attach_to, type))
...
class KeyboardControl:
    ...
    def parse_events(self):
        self.parse_vehicle_keys(pygame.key.get_pressed(), 100)
...
if __name__ == "__main__":
    ...
    try:
        ...
        spawn_points = [carla.Transform(carla.Location(x= box.location.x - 4 ,y= box.location.y , z=box.location.z ), carla.Rotation(0,0,0)),
                        carla.Transform(carla.Location(x= box.location.x+box.extent.x, y= box.location.y - box.extent.y, z=box.location.z+box.extent.z), carla.Rotation(0,0,0)),
                        carla.Transform(carla.Location(x= box.location.x+box.extent.x, y= box.location.y + box.extent.y, z=box.location.z+box.extent.z), carla.Rotation(0,0,0))]
        render_pos = [(0,360), (0,0), (640,0)]



        for i in range(1):
            world.spawn_camera(spawn_points[i], (0,0), (640, 360), attach_to=world.vehicle, type = 0)
        ...
        box = world.vehicle.bounding_box
        mcp = [(carla.Location(x = -10 , y= 0 , z = 2), carla.Rotation(-20,0,0)), (carla.Location(x = 0 , y= -5 , z = 2), carla.Rotation(-20,90,0)), (carla.Location(x = 5 , y= 2 , z = 2), carla.Rotation( -20,200,0)), (carla.Location(x= box.location.x+1 ,y= box.location.y , z=box.location.z), carla.Rotation(0,0,0)) ]
        mcp_index = 0
